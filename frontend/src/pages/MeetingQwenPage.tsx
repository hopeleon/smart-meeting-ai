...
export default function MeetingQwenPage() {
  ...
  useEffect(() => {
    ...
    getMeeting(meetingId).then((meeting) => {
      ...
      const ws = new WhisperWebSocket(meetingId, 'qwen')
      wsRef.current = ws

      ws.onMessage((msg: Record<string, unknown>) => {
        console.log('[QwenWS-收到]', JSON.stringify(msg).slice(0, 300))
        ...
        if (msg.type === 'transcript.completed') {
          ...
          console.log('[QwenWS] 收到 ready_to_stop，服务端处理完毕')
  ...
  const startRecording = async (source: AudioSource) => {
    ...
    try {
      ...
      // AudioContext 设为 16kHz（Qwen 原生采样率，不需要降采样）
      ...
      <div className="flex-1 flex gap-4 min-h-0">
        <div className="flex w-1/3 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-900 shadow-sm">
          <div className="border-b border-slate-200 px-4 py-3">
            <h2 className="text-sm font-semibold text-slate-950">实时转写 (Qwen)</h2>
          </div>
